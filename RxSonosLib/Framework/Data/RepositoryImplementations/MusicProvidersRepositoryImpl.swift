...
class MusicProvidersRepositoryImpl: MusicProvidersRepository {
    ...
    func getMusicProviders(for room: Room) -> Single<[MusicProvider]> {
        return network
            ...
            .map(mapDataToMusicServices)
    }
    
}

private extension MusicProvidersRepositoryImpl {
    func mapDataToMusicServices(data: [String: String]) throws -> [MusicProvider] {
        let xml = try AEXMLDocument.create(data["AvailableServiceDescriptorList"])
        return xml?["Services"].children.compactMap(mapService) ?? []
    }
